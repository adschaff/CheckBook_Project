...
# This function will render the Balance page when requsted
def balance(request, pk):
    ...
    current_total = account.initial_deposit  # Creates account variable, starting with the initial deposit value
    table_contents = {}  # creates a dictionary into which transaction infromation will be placed
...
def transaction(request):
    ...
    if request.method == 'POST':
        if form.is_valid():  # Check to see if the submitted form is valid and if so, saves the form
            ...
            return balance(request, pk)  # Returns balance of Balance sheet
    content = {'form': form}  # Saves content to the template as a dictionary
    # adds content of form to page
